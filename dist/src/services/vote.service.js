...
export async function createVote(slug, voterToken, selectedOptionIds = [], selectedDays = []) {
    ...
    return transaction(async (client) => {
        const result = await client.query(`select p.*, coalesce(json_agg(json_build_object('id', o.id))
      filter (where o.id is not null), '[]') options from polls p left join poll_options o on o.poll_id = p.id
      where p.slug = $1 group by p.id`, [slug]);
        ...
        try {
            await client.query("insert into votes (id, poll_id, voter_token_hash) values ($1, $2, $3)", [voteId, poll.id, hashToken(voterToken)]);
        }
        catch (error) {
            if (error.code === "23505")
        ...
        for (const optionId of optionIds)
            await client.query("insert into vote_options (id, vote_id, option_id) values ($1, $2, $3)", [randomUUID(), voteId, optionId]);
        for (const day of days)
            await client.query("insert into vote_days (id, vote_id, date) values ($1, $2, $3)", [randomUUID(), voteId, dateKey(poll.year, poll.month, day)]);
        return { id: voteId };
    });
}
export async function getResults(slug) {
    const pollResult = await pool.query("select slug, question, type, month, year, status from polls where slug = $1", [slug]);
    ...
    const [participants, options, days] = await Promise.all([
        ...
        pool.query(`select o.id, o.text, count(vo.id)::int count from poll_options o left join vote_options vo on vo.option_id = o.id
      where o.poll_id = $1 group by o.id order by o.created_at`, [id]),
        pool.query(`select vd.date::text date, count(*)::int count from vote_days vd join votes v on v.id = vd.vote_id
      where v.poll_id = $1 group by vd.date order by vd.date`, [id]),
    ]);
